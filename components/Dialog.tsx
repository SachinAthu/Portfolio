...
function DialogCloseBtn({ onClose }: { onClose: () => void }) {
  return (
    <button
      type="button"
      onClick={onClose}
      className="relative z-10 h-8 w-8 before:absolute before:top-1/2 before:left-0 before:h-1 before:w-full before:-translate-y-1/2 before:-rotate-45 before:rounded-full before:bg-gray-300 before:transition-colors before:duration-500 before:content-[''] after:absolute after:top-1/2 after:left-0 after:h-1 after:w-full after:-translate-y-1/2 after:rotate-45 after:rounded-full after:bg-gray-600 after:transition-colors after:duration-500 after:content-[''] hover:before:bg-gray-600 hover:after:bg-gray-300"
  ...
  ({ children, id, ariaLabel, imageView = false }: DialogProps, ref) => {
    ...
    return (
      <dialog
        ...
        ref={dialogElRef}
        className={cn(
          "bg-background dark:bg-d-background relative overflow-hidden",
          imageView

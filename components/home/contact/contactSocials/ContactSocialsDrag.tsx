...
const SocialLink = memo(
  ({ index, id, title, link, xVal, handleMouseUp }: SocialLinkProps) => {
    ...
    useEffect(() => {
      ...
      let minX = 0;
      let maxX =
        socialLinkWrapperEl.current.clientWidth -
        socialLinkEl.current.clientWidth -
        8; // padding x = 8px
      ...
      const xTo = gsap.quickTo(socialLinkEl.current, "x", {
        duration: 0.6,
        ease: "power2.out",
      });
      const scaleXTo = gsap.quickTo(socialLinkBorderEl.current, "scaleX", {
        duration: 0.6,
        ease: "power2.out",
      });
      ...
      // mouse events
      socialLinkEl.current.addEventListener("mousedown", onMouseDown, {
      ...
      socialLinkEl.current.addEventListener("mouseleave", onMouseLeave, {
        signal: controller.signal,
      });

      // touch events
      if (isTouch) {
        socialLinkEl.current.addEventListener("touchstart", onMouseDown, {
          signal: controller.signal,
        });
    ...
    return (
      <div
        ref={socialLinkWrapperEl}
        className="social-link-container overflow-hidden px-1 pt-6">
        <Link
          ref={socialLinkEl}
          id={id}
          href={link}
          ...
          className="social-link | group relative grid h-20 w-36 cursor-grab touch-pan-x place-items-center overflow-hidden rounded-full border border-gray-300 px-1 text-base will-change-transform select-none sm:w-44 sm:text-lg lg:h-28 lg:w-56 lg:text-2xl dark:border-gray-600">
          <div className="social-link-inner | relative origin-[50%_0]">
            <div className="social-link-inner-1 | whitespace-nowrap transition-colors duration-300 group-hover:text-(--color-custom-btn-hover)">
              {title}
            </div>

            <div className="social-link-inner-2 | absolute origin-[50%_0] [&_svg]:h-8 [&_svg]:w-8 [&_svg]:will-change-transform group-hover:[&_svg]:fill-(--color-custom-btn-hover) lg:[&_svg]:h-10 lg:[&_svg]:w-10">

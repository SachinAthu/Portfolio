...
export default function IconButton({ children, className, onClick, direction = 'up', ...rest }: IconButtonProps) {
  return (
    <button
      type="button"
      className={cn(
        'custom-icon-button | relative block overflow-hidden rounded-full border border-text p-2 dark:border-d-text [&_svg]:h-6 [&_svg]:w-6',
        direction === 'down' ? 'down' : '',
        className || ''
      )}
      onClick={onClick}
      ...
      <span className="flair | pointer-events-none absolute left-0 top-0 z-[-1] aspect-[1] w-[300%] -translate-x-1/2 -translate-y-1/2 rounded-full will-change-transform"></span>
    </button>
  );
}
